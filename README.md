...
门诊用药决策中，最消耗医生时间、最容易出风险的，往往不是常见病，而是那些**现有系统回答不了的问题**。当医生面对以下场景时，现有药品系统通常只能给出基础信息，真正的判断仍依赖医生自己跨库查询、人工核对：

**场景一：罕见病与孤儿药**

一名内分泌科医生接诊罕见病患者，需要用到院内极少使用的孤儿药。医院药品知识库几乎没有相关条目，医生只能手动查询，甚至手机上查询检索，并人工核对剂量和适应证。
...
- 信息分散在药品系统、检验系统、外网APP、医学数据库和文献之间；
- 单次查询无法覆盖多源信息，需要反复切换、人工综合；
- 医院数据不能出内网，公网大模型和在线数据库无法直接联用；
- 最终决策必须可溯源、可审批，但手动查询过程难以留痕。

**Med Agent 智能医疗辅助平台** 就是为这些"现有系统搞不定"的场景设计的私有化AI助手：它通过RAG把医院本地知识库（药品说明书、临床指南、医学文献、历史病历）统一接入，基于LangGraph主动追问患者信息，借助多Agent协作模拟多科室会诊，最终生成可溯源、可审批的用药建议。

### 1.2 能力概述

Med Agent 是一个基于 **RAG + LangGraph 的多 Agent 协作医学问答平台**。医生只需要像跟同事说话一样，对着对话框输入问题，系统自动完成以下事情：

- **并行检索**药品说明书、临床指南、医学文献、药物相互作用库、历史病历
- **综合所有来源**生成回答，每条信息标注出处
- **主动追问**缺失的患者信息（年龄、过敏史、用药史）
- **生成用药评估表**并自动进入审批流程
- **沉淀经验**，审批通过的方案后回写历史库，供续相似病例参考

### 1.3 一句话总结

> 把原来需要切换多个系统、耗时数分钟才能查到的医学信息，变成一句自然语言对话，由系统自动并行检索、综合来源、主动追问，最终给出有据可查的用药建议。AI负责检索、初筛、汇总，医生做最终决策。
